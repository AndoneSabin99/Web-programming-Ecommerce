...
public class BuyServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (creditCard.equals((String)session.getAttribute("creditCard"))){

            Carrello cart = (Carrello)session.getAttribute("carrello");
            
            Iterator<Product> iter = cart.cart.iterator();
            while (iter.hasNext()) {
                Product product = iter.next();
                iter.remove();
            }      
            String message = "L'acquisto è stato effettuato con successo!";
            request.setAttribute("messageSuccess", message);
            session.setAttribute("carrello",cart);            
            request.getRequestDispatcher("Homepage.jsp").forward(request, response);
        } else{
            String message = "Carta di credito sbagliata";
